...
void callFunction(char *funcName, char **args, int argCount) {
    ...
    // UNCOMMENT ME AFTER YOU HAVE WRITTEN MY FUNCTION
        ...
        else if (strcmp(funcName, "add_like") == 0) {
        if (argCount >= 2) {
            add_like(searchDatabase(args[1]),search_user_by_name(args[0]));
        } else {
            printf("Invalid arguments for %s\n", funcName);
        }
    }
...
int main() {
    FILE *file = fopen("C:/Users/kaden/OneDrive - Queen's University/Second Year/Fall term/ELEC278 Fundamentals of Info Structure/assignment_starter_files/ASSIGNMENT 2024/test_cases.csv", "r");
    if (file == NULL) {
